...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        x_cap = (x - running_mean) / running_std
    ...
    for n_sample in range(N):
        ...
        for h_out in range(out.shape[2]):
            ...
            for w_out in range(out.shape[3]):
                out[n_sample, :, h_out, w_out] = (x[n_sample, :, hp: hp + HH, wp: wp + WW] * w).sum(axis=(1, 2, 3)) + b
                wp += stride
            hp += stride
...
def conv_backward_naive(dout, cache):
    ...
    for n_sample in range(N):
        for n_filter in range(F):
            hp = 0
            for h_out in range(dout.shape[2]):
                wp = 0
                for w_out in range(dout.shape[3]):
                    dout_curr = dout[n_sample, n_filter, h_out, w_out] * np.ones(dw.shape[1:])
                    dw[n_filter] += x[n_sample, :, hp: hp + HH, wp: wp + WW] * dout_curr

                    dx[n_sample, :, hp: hp + HH, wp: wp + WW] += w[n_filter] * dout_curr
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    x_trasposed = np.transpose(x, (0, 2, 3, 1))
    x_reshaped = x_trasposed.reshape((-1, C))
    out_reshaped, cache = batchnorm_forward(x_reshaped, gamma, beta, bn_param)
    out_trasposed = out_reshaped.reshape((N, H, W, C))
    out = np.transpose(out_trasposed, (0, 3, 1, 2))
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dout_trasposed = np.transpose(dout, (0, 2, 3, 1))
    dout_reshaped = dout_trasposed.reshape((-1, C))
    dx_reshaped, dgamma, dbeta = batchnorm_backward_alt(dout_reshaped, cache)
    dx_trasposed = dx_reshaped.reshape((N, H, W, C))
    dx = np.transpose(dx_trasposed, (0, 3, 1, 2))
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    (N, C, H, W) = x.shape
    x_group_shape = (N * G, C // G * H * W)
    x_group = x.reshape((N * G, C // G * H * W))
    x_mean = x_group.mean(axis=1, keepdims=True)
    x_var = x_group.var(axis=1, keepdims=True)
    x_centered = (x_group - x_mean)
    x_inv_std = 1. / np.sqrt(x_var + eps)
    x_cap = x_centered * x_inv_std

    x_cap = x_cap.reshape(x.shape)
